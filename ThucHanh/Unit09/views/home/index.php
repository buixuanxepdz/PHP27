<!DOCTYPE html>
<html class="no-js" lang>
<head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title>Bùi Xuân Xếp</title>
    <meta name="description" content="Maverick - Minimalist Blog Magazine">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <link rel="apple-touch-icon" href="images/apple-touch-icon.png">
    <!-- Place favicon.ico in the root directory -->

    <!-- /Google Fonts -->
    <link href="public/css/css.css" rel="stylesheet" type="text/css">
    <link href="public/css/css_1.css" rel="stylesheet" type="text/css">
    <link href="public/css/css_2.css" rel="stylesheet" type="text/css">

    <!-- /Font Icons -->
    <link rel="stylesheet" href="public/css/font-awesome.min.css">


    <!-- /Plugins CSS -->
    <link rel="stylesheet" href="public/css/slick.css">
    <link rel="stylesheet" href="public/css/perfect-scrollbar.min.css">

    <!-- /CSS -->
    <link rel="stylesheet" href="public/css/bootstrap.min.css">
    <link rel="stylesheet" href="public/css/reset.css">
    <link rel="stylesheet" href="public/css/normalize.css">
    <link rel="stylesheet" href="public/css/main.css">

    <!-- /JS -->
    <script src="js/modernizr.min.js"></script>

    <script>
      (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
          (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
          m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
      })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

      ga('create', 'UA-72744907-1', 'auto');
      ga('send', 'pageview');

  </script>
</head>
<body class="sticky-nav-on">
        <!--[if lt IE 8]>
            <p class="browserupgrade">You are using an <strong>outdated</strong> browser. Please <a href="http://browsehappy.com/">upgrade your browser</a> to improve your experience.</p>
        <![endif]-->

        <div class="sticky-nav">
            <div class="container">
                <div class="row">
                    <div class="col-md-12">
                        <div class="logo-container">
                            <a href="#">
                                <img src="images/logo3.png" alt="Maverick Logo">
                            </a>
                        </div><!-- /logo-container -->
                    </div><!-- /col-md-12 -->
                </div><!-- /row -->
            </div><!-- /container -->
        </div><!-- /sticky-nav -->


        <aside id="sideheader">
            <header>
                <div class="logo-container">
                    <a href="#"><img src="images/logo.png" alt="Maverick"></a>
                </div><!-- /logo-container -->
                <a href="#" class="sideheader-close-btn">
                    <span></span>
                    <span></span>
                </a>
            </header>
            <div class="widget widget_nav_menu">
                <ul>
                    <li><a href="/page-category.html">Popular</a></li>
                    <li><a href="/page-category.html">Fashion</a></li>
                    <li><a href="/page-category.html">Lifestyle</a></li>
                    <li><a href="/page-category.html">Arts</a></li>
                    <li><a href="/page-category.html">Design</a></li>
                    <li><a href="/page-category.html">Music</a></li>
                    <li><a href="/page-category.html">Entertainment</a></li>
                    <li><a href="/page-category.html">Travel</a></li>
                    <li><a href="/page-category.html">Gadgets</a></li>
                    <li><a href="/page-category.html">Food</a></li>
                </ul>
            </div>
            <div class="widget widget_nav_menu">
                <ul>
                    <li><a href="#">About</a></li>
                    <li><a href="page-contact.html">Contact</a></li>
                    <li><a href="terms.html">Terms</a></li>
                </ul>
            </div>
            <div class="widget widget_socials">
                <ul class="social-icons small">
                    <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                    <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                    <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                    <li><a href="#"><i class="fa fa-pinterest"></i></a></li>
                    <li><a href="#"><i class="fa fa-google-plus"></i></a></li>
                    <li><a href="#"><i class="fa fa-tumblr"></i></a></li>
                    <li><a href="#"><i class="fa fa-youtube-play"></i></a></li>
                    <li><a href="#"><i class="fa fa-envelope-o"></i></a></li>
                </ul>
            </div>
            <div class="widget widget_customAd">
                <a href="#">
                    <img src="images/sideheaderAd.jpg" alt="Maverick">
                </a>
            </div>
            <p class="copyright">Copyright © 2021 BXX</p>
        </aside><!-- /sideheader -->

        <section id="wrapper">

            <div class="top-nav-wrapper">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12">
                            <a href="#" class="nav-trigger">
                                <span class="bars">
                                    <span></span>
                                    <span></span>
                                    <span></span>
                                </span>
                            </a>
                            <ul class="top-nav">
                                <li><a href="#"><i class="fa fa-home"></i></a></li>
                               <!--  <li><a href="about.html">About</a></li>
                                <li><a href="page-contact.html">Contact</a></li>
                                <li><a href="#">Terms</a></li> -->
                            </ul>
                            <div class="search-container">
                                <a href="#" class="trigger">
                                    <i class="fa fa-search"></i>
                                </a>
                                <form action="#" class="header-search-form">
                                    <div class="input-container">
                                        <input type="search" placeholder="Search..">
                                    </div>
                                </form>
                            </div><!-- /search-container -->
                        </div><!-- /col-md-12 -->
                    </div><!-- /row -->
                </div><!-- /container -->
            </div><!-- /top-nav-wrapper -->
            <header id="main-header">
                <div class="container">
                    <div class="row">
                        <div class="col-md-12">
                            <nav class="main-nav clearfix">
                                <a href="#" class="mobile-nav-trigger">
                                    <span class="bars">
                                        <span></span>
                                        <span></span>
                                        <span></span>
                                    </span>
                                </a>
                                <div class="logo-wrapper">
                                    <a href="index.html" class="logo">
                                        <img src="images/logo.png" alt="Maverick Logo">
                                    </a>
                                </div><!-- /logo-wrapper -->
                                <div class="search-container">
                                    <a href="#" class="trigger">
                                        <i class="fa fa-search"></i>
                                    </a>
                                    <form action="#" class="header-search-form">
                                        <div class="input-container">
                                            <input type="search" placeholder="Search..">
                                        </div>
                                    </form>
                                </div><!-- /search-container -->
                                <ul class="main-nav-items">
                                    <li><a href="page-category.html">Fashion</a></li>
                                    <li><a href="page-category.html">Sports</a></li>
                                    <li><a href="page-category.html">Music</a></li>
                                    <li><a href="page-category.html">Travel</a></li>
                                </ul>
                                <ul class="social-icons">
                                    <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                                    <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                                    <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                                    <li><a href="#"><i class="fa fa-pinterest"></i></a></li>
                                    <li><a href="#"><i class="fa fa-youtube"></i></a></li>
                                </ul>
                            </nav>
                        </div><!-- /col-md-12 -->
                    </div><!-- /row -->
                </div><!-- /container -->
            </header>

            <section class="contents-container">
                <div class="container">
                    <div class="row">
                        <div class="col-md-8">
                            
                            <div class="blog-post-slider">
                                <?php foreach ($threePost as $key => $value) {
                                # code...
                             ?>
                                <article class="blog-post">
                                    <header>
                                        <figure>
                                            <img style="width: 100%;height: 400px;" src="<?= $value['thumbnail']; ?>" alt>
                                        </figure>
                                        <ul class="categories">
                                            <li><a href=""><?= $value['category_name']; ?></a></li>
                                        </ul>
                                        <h3><a href="index.php?mod=post&act=detail&id=<?= $value['id']?>"><?= $value['title']; ?></a></h3>
                                        <h4><a href="index.php?mod=post&act=detail&id=<?= $value['id']?>"><?= $value['description']; ?></a></h4>
                                    </header>
                                    <footer>
                                        <div class="meta">
                                            <span><time datetime="2015-09-03"><?= $value['created_at']; ?></time></span>
                                            <span>6052 Views</span>
                                            <span><a href="#">8 Comments</a></span>
                                        </div><!-- /meta -->
                                    </footer>
                                </article>
                                  <?php } ?>
                            </div><!-- /slider -->
                      
                        </div><!-- /col-md-8 -->
                        <div class="col-md-4">
                             <?php foreach ($twoPost as $key => $value) {
                                # code...
                             ?>
                            <article class="blog-post featured-post">
                                <header>
                                    <figure>
                                        <img src="<?= $value['thumbnail'];?>" alt="Maverick Blog">
                                    </figure>
                                    <h3><a href="index.php?mod=post&act=detail&id=<?= $value['id']?>"><?= $value['title']; ?></a></h3>
                                </header>
                            </article>
                            <?php } ?>
                        </div><!-- /col-md-4 -->
                        <div class="col-md-8">
                            <div class="blog-tabs clearfix">
                                <a href="#latest-posts" class="active">Latest Posts</a>
                                <a href="#popular-posts">Most Views</a>
                            </div><!-- /page-titlebar -->
                            <div id="latest-posts" class="tab-contents active">
                                <div class="contents-inner grid-view clearfix">
                                   <?php foreach ($get6Post as $key => $value) {
                                # code...
                                       ?>
                                       <article class="blog-post col-md-6 col-sm-6">
                                        <header>
                                            <figure>
                                                <img style="width: 100%;height: 200px;" src="<?= $value['thumbnail'] ?>" alt="Maverick Blog">
                                            </figure>
                                            <ul class="categories">
                                                <li><a href="#"><?= $value['category_name']; ?></a></li>
                                            </ul>
                                            <h3><a href="index.php?mod=post&act=detail&id=<?= $value['id']?>"><?= $value['title']; ?></a></h3>
                                            <div class="meta">
                                                <span><?= $value['created_at']; ?></span>
                                                <span>6052 Views</span>
                                                <span><a href="#">8 Comments</a></span>
                                            </div><!-- /meta -->
                                        </header>
                                        <div class="post-content">
                                            <p><?= $value['description']; ?></p>
                                        </div><!-- /post-content -->
                                    </article>
                                <?php } ?>
                                
                                </div><!-- /contenblog-popular-poststs-inner -->
                            </div><!-- /blog-latest-posts -->
                            <div id="popular-posts" class="tab-contents">
                                <div class="contents-inner grid-view clearfix">
                                     <?php foreach ($viewcount as $key => $value) {
                                # code...
                                       ?>
                                    <article class="blog-post col-md-6 col-sm-6">
                                        <header>
                                            <figure>
                                                <img style="width: 100%;height: 200px;" src="<?= $value['thumbnail'] ?>" alt="Maverick Blog">
                                            </figure>
                                            <ul class="categories">
                                                <li><a href="#"><?= $value['category_name'] ?></a></li>
                                            </ul>
                                            <h3><a href="index.php?mod=post&act=detail&id=<?= $value['id']?>"><?= $value['title'] ?></a></h3>
                                            <div class="meta">
                                                <span><?= $value['created_at'] ?></span>
                                                <span>6052 Views</span>
                                                <span><a href="#">8 Comments</a></span>
                                            </div><!-- /meta -->
                                        </header>
                                        <div class="post-content">
                                            <p><?= $value['description'] ?></p>
                                        </div><!-- /post-content -->
                                    </article>
                                    <?php } ?>
                                </div><!-- /contents-inner -->
                            </div><!-- /blog-popular-posts -->
                            <div class="blog-navigation clearfix">
                                <a href="#" class="ajax-load-more">Load More</a>
                            </div><!-- /blog-navigation -->
                        </div><!-- /col-md-8 -->
                        <div class="col-md-4">
                            <div class="sidebar">
                                <div class="widget widget_search">
                                    <form role="search" class="search-form" action="#">
                                        <input type="search" class="search-field" name="s" title="Search for:" placeholder="Search…">
                                        <input type="submit" value="&#xf002;">
                                    </form>
                                </div><!-- /widget -->
                                <div class="widget widget_socials">
                                    <h5>Social Media</h5>
                                    <ul class="social-icons small">
                                        <li><a href="#"><i class="fa fa-facebook"></i></a></li>
                                        <li><a href="#"><i class="fa fa-twitter"></i></a></li>
                                        <li><a href="#"><i class="fa fa-instagram"></i></a></li>
                                        <li><a href="#"><i class="fa fa-pinterest"></i></a></li>
                                        <li><a href="#"><i class="fa fa-google-plus"></i></a></li>
                                        <li><a href="#"><i class="fa fa-tumblr"></i></a></li>
                                        <li><a href="#"><i class="fa fa-youtube-play"></i></a></li>
                                        <li><a href="#"><i class="fa fa-vine"></i></a></li>
                                        <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
                                        <li><a href="#"><i class="fa fa-envelope-o"></i></a></li>
                                    </ul>
                                </div><!-- /widget -->
                                <div class="widget widget_latest_posts_entries">
                                    <h5>Similar articles</h5>
                                     <?php foreach ($random as $key => $value) {
                                # code...
                                       ?>
                                    <ul>
                                        <li>
                                         <div class="featured-image">
                                            <img style="width: 90px;height: 70px;" src="<?= $value['thumbnail'] ?>" alt="Maverik Sidebar">
                                        </div>
                                        <div class="post-content">
                                            <p class="post-title"><a href="index.php?mod=post&act=detail&id=<?= $value['id']?>"><?= $value['title'] ?></a></p>
                                            <span class="category"><?= $value['category_name'] ?></span>
                                            <span class="post-date">August 5, 2015</span>
                                        </div>
                                    </li>

                                    </ul>
                                <?php } ?>
                    </div><!-- /widget -->
                    <div class="widget widget_categories">
                        <h5>Categories</h5>
                        <ul>
                            <li><a href="#">Travel</a></li>
                            <li><a href="#">Sports</a> (10)</li>
                            <li><a href="#">Music</a> (6)</li>
                            <li><a href="#">Fashion</a> (2)</li>
                        </ul>
                    </div>/widget
                    <div class="widget widget_tag_cloud">
                        <h5>Tag Cloud</h5>
                        <div class="tagcloud">
                            <a href="#">Travel</a>
                            <a href="#">Fashion</a>
                            <a href="#">Automative</a>
                            <a href="#">Music</a>
                            <a href="#">Music</a>
                            <a href="#">Automative</a>
                            <a href="#">Fashion</a>
                            <a href="#">Travel</a>
                            <a href="#">Travel</a>
                            <a href="#">Fashion</a>
                            <a href="#">Automative</a>
                            <a href="#">Music</a>
                        </div>
                    </div><!-- /widget
                    <div class="widget widget_text">
                        <h5>Basic Text</h5>
                        <div class="textwidget">
                            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
                        </div>
                    </div><!-- /widget -->
                </div><!-- /sidebar -->
            </div><!-- /col-md-4 -->
        </div><!-- /row -->
    </div><!-- /container -->
</section>

<footer id="main-footer">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <div class="widget widget_text">
                    <h5>About Us</h5>
                    <div class="textwidget">
                        <img src="images/logo2.png" alt="Maverick">
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
                        <form class="subscribe-form" action="php/subscribe.php">
                            <input type="email" class="subscribe-email" name="subscribe-email" placeholder="Sign Up For Our Newsletter">
                            <input type="submit" value="&#xf101;">
                            <label class="subscribe-form-result"></label>
                        </form>
                    </div>
                </div><!-- /widget -->
            </div><!-- /col-md-4 -->
            <div class="col-md-4">
                <div class="widget widget_latest_posts_entries">
                    <h5>Latest Posts</h5>
                     <?php foreach ($random as $key => $value) {
                                # code...
                       ?>
                       <ul>
                        <li>
                           <div class="featured-image">
                            <img style="width: 90px;height: 70px;" src="<?= $value['thumbnail'] ?>" alt="Maverik Sidebar">
                        </div>
                        <div class="post-content">
                            <p class="post-title"><a href="index.php?mod=post&act=detail&id=<?= $value['id']?>"><?= $value['title'] ?></a></p>
                            <span class="category"><?= $value['category_name'] ?></span>
                            <span class="post-date">August 5, 2015</span>
                        </div>
                    </li>
                    
                        </ul>
                    <?php } ?>
            </div><!-- /widget -->
        </div><!-- /col-md-4 -->
<div class="col-md-4">
    <div class="widget widget_categories">
        <h5>Categories</h5>
        <ul>
           <li><a href="#">Travel</a></li>
           <li><a href="#">Sports</a> (10)</li>
           <li><a href="#">Music</a> (6)</li>
           <li><a href="#">Fashion</a> (2)</li>
        </ul>
    </div><!-- /widget -->
</div><!-- /col-md-4 -->
</div><!-- /row -->
</div><!-- /container -->
</footer>

<div id="bottom-footer">
    <ul class="social-icons small">
        <li><a href="#"><i class="fa fa-facebook"></i></a></li>
        <li><a href="#"><i class="fa fa-twitter"></i></a></li>
        <li><a href="#"><i class="fa fa-instagram"></i></a></li>
        <li><a href="#"><i class="fa fa-pinterest"></i></a></li>
        <li><a href="#"><i class="fa fa-google-plus"></i></a></li>
        <li><a href="#"><i class="fa fa-tumblr"></i></a></li>
        <li><a href="#"><i class="fa fa-youtube-play"></i></a></li>
        <li><a href="#"><i class="fa fa-vine"></i></a></li>
        <li><a href="#"><i class="fa fa-linkedin"></i></a></li>
        <li><a href="#"><i class="fa fa-envelope-o"></i></a></li>
    </ul>
    <p class="copyright">Copyright © 2021 BXX</p>
</div><!-- /bottom-footer -->

</section><!-- /wrapper -->

<script src="public/js/jquery.min.js"></script>
<script src="public/js/plugins.js"></script>
<script src="public/js/main.js"></script>

</body>
</html>
